...
static void begin_transaction() {
	ST7528_1_SS_Reg_Write(0);
}

static void end_transaction() {
	ST7528_1_SS_Reg_Write(1);
}
static void send_commands(uint8 commands[], int len) {
	begin_transaction();
	ST7528_1_SPI_PutArray(commands, len);
	end_transaction();
}
...
static void display_setup() {
	...
	ST7528_1_SetContrast(DEFAULT_CONTRAST_LEVEL);
...
void write_pixels_begin(uint8 len) {
	begin_transaction();
	ST7528_1_SPI_PutArray((uint8[]){COMMAND_WRITE_DATA, len - 1}, 2);
}
...
void ST7528_1_WritePixels(uint8 data[], uint8 len) {
	...
	ST7528_1_SPI_PutArray(data, len);
	write_pixels_end();
}
...
static void draw_text_slice(char c, uint8 row) {
	...
	for(int8 i = 0; i < FONT_GLYPH_COLUMNS; i++) {
		...
		ST7528_1_SPI_PutArray(pixels, 4);
	}
	write_pixels_end();
}
...
void ST7528_1_Clear(uint8 start_row, uint8 start_col, uint8 end_row, uint8 end_col) {
	for(uint8 row = start_row; row < end_row; row++) {
		ST7528_1_SetCursorPosition(row, start_col);
		for(uint8 col = start_col; col < end_col; col++) {
			ST7528_1_WritePixels((uint8[]){0, 0, 0, 0}, 4);
		}
	}
}

void ST7528_1_ClearAll() {
	ST7528_1_Clear(0, 0, 8, 160);
}

void ST7528_1_DrawText(uint8 start_page, uint8 start_col, char *text) {
	for(uint8 row = 0; row < 2; row++) {
		ST7528_1_SetCursorPosition(start_page + row, start_col);
		for(char *c = text; *c != '\0'; c++) {
			draw_text_slice(*c, row);
		}
	}
}
...
void ST7528_1_DrawBigNumbers(uint8 start_page, uint8 start_col, char *nums) {
	...
	for(uint8 vglyph = 0; vglyph < 3; vglyph++) {
		for(uint8 row = 0; row < 2; row++) {
			ST7528_1_SetCursorPosition(start_page + vglyph * 2 + row, start_col);
				...
				for(uint8 hglyph = 0; hglyph < 3; hglyph++) {
					if(*c >= '0' && *c <= '9') {
						...
						uint8 glyphnum = FONT_BIGDIGIT_OFFSET
										 ...
										 + hglyph;
						draw_text_slice(glyphnum, row);
					} else if(*c == '.') {
						hglyph = 2; // Only one glyph wide for '.'
						if(vglyph == 2) {
							draw_text_slice(FONT_BIGDIGIT_PERIOD, row);
						} else {
							draw_text_slice(' ', row);
						}
					}
